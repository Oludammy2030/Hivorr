...
OnboardingTestStack buildOnboardingStack({
  ...
  final EntityRepositoryImpl entityRepository = EntityRepositoryImpl(
    remote: r,
    local: local,
  );
  final EntityProvider entityProvider = EntityProvider(
  ...
  final FakeStorageService storageService = storage ?? FakeStorageService();
...
HiveOnboardingProgressStore hiveProgressStore({LocalStore? store}) =>
    HiveOnboardingProgressStore(
      store: store ?? LocalStore(FakeStorageEngine()),
    );
