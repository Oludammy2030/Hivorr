...
void main() {
  ...
  group('submitTradeProof', () {
    test(
      'validates, uploads, inserts + queues for a signed-in entity',
      () async {
    ...
    test('builds a per-submission storage path under the entity id', () async {
      ...
      expect(
        firstPath,
        isNot(secondPath),
        reason: 'each submit has a new UUID',
      );
    });
  ...
  group('credential insert (server-authoritative)', () {
    ...
    buildRecording({TradeProfessionNameResolver? resolveName}) {
      ...
      final _RecordingSupabaseClient client = _RecordingSupabaseClient(
        recorder,
      );
    ...
    test(
      'insert binds profession_id + trade kind and resolves the title',
      () async {
    ...
    test(
      'title falls back to the proof label when the resolver is null',
      () async {
    ...
    test(
      'title falls back to the label for an empty resolver result',
      () async {
    ...
    test(
      'insert never writes verification_status or reviewed_at (Rule 4)',
      () async {
        final built = buildRecording(resolveName: (_) async => 'Plumbing');
  ...
  group('getStatus', () {
    ...
    test(
      'never forwards a client-supplied entity id (server derives it)',
      () async {
        final remote = FakeTradeVerificationRemoteDataSource(
          statusResult: tradeStatusDto(
            identityVerified: true,
...
class _RecordingHttpClient extends ScriptedHttpClient {
  _RecordingHttpClient({super.queryResults});

  final List<({String path, String body})> posts =
...
class _RecordingSupabaseClient extends SupabaseClient {
  ...
  final FakeGoTrueClient _goTrue = FakeGoTrueClient()
    ..seedSession(fakeSession('u1'));
}
