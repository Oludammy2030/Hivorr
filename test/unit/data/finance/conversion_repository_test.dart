...
void main() {
  ...
  group('getRate', () {
    ...
    test(
      'validates the pair before delegating (unsupported currency, PLT003)',
      () async {
        final rateSource = FakeConversionRateSource();
        final repo = build(rateSource: rateSource);
    ...
    test(
      'rejects a self-pair as validation error before any seam call',
      () async {
        final rateSource = FakeConversionRateSource();
        final repo = build(rateSource: rateSource);
    ...
    test('fails closed when the seam has no configured rate', () async {
      ...
      final rateSource = FakeConversionRateSource()..nextError = e;
      final repo = build(rateSource: rateSource);
  ...
  group('previewConversion', () {
    test(
      'computes the local estimate against the trusted rate with zero RPCs',
      () async {
        final remote = FakeConversionRemoteDataSource();
        final rateSource = FakeConversionRateSource()
          ..setRate('NGN', 'USD', 0.0007);
    ...
    test(
      'applies the injected fee to the net (toAmount = gross - fee)',
      () async {
        final rateSource = FakeConversionRateSource()
          ..setRate('NGN', 'USD', 0.0007);
    ...
    test('fails closed when the seam returns a non-positive rate', () async {
      final repo = build(rateSource: _ZeroRateSource());
  ...
  group('executeConversion', () {
    ...
    test(
      'surfaces PLT006 insufficient balance as a conflict ApiException',
      () async {
        final remote = FakeConversionRemoteDataSource()
    ...
    test('a balance-refresh failure never fails an already-committed '
      ...
      final rateSource = FakeConversionRateSource()
        ..setRate('NGN', 'USD', 0.0007);
      final repo = build(rateSource: rateSource, financial: financial);
    ...
    test(
      'fails closed before any RPC when the seam rate is unavailable',
      () async {
        final remote = FakeConversionRemoteDataSource();
    ...
    test('maps remote DTOs to entities in order', () async {
      final remote = FakeConversionRemoteDataSource(
        history: <CurrencyConversionDto>[
          ...
          seedConversionDto(id: 'c-2', fromCurrency: 'USD', toCurrency: 'GHS'),
        ],
      );
