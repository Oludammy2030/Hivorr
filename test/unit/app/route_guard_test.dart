...
void main() {
  group('RouteGuard', () {
    group('entry doors', () {
      test(
        'unauthenticated root resolves to the platform entry door',
        () async {
      ...
      test(
        'native returning visitor (intro seen) is routed to /login',
        () async {
          if (kIsWeb) {
            return;
          }
    ...
    test('authenticated users may open admin routes (router is not an admin '
        'gate)', () {
      final provider = FakeAuthProvider(
        initialStatus: AuthStatus.authenticated,
      );
  ...
  group('RouteGuard (development-only onboarding preview)', () {
    test(
      'development lets an unauthenticated reviewer open onboarding routes',
      () {
        final provider = FakeAuthProvider(
          initialStatus: AuthStatus.unauthenticated,
        );
    ...
    test(
      'default (production) guard stays fail-closed on onboarding routes',
      () {
        final provider = FakeAuthProvider(
          initialStatus: AuthStatus.unauthenticated,
        );
    ...
    test('development boot redirect supports onboarding sub-step deep links '
  ...
  group('RouteGuard (exit-and-resume gate)', () {
    ...
    test(
      'lifecycle saveAndExit never suppresses the resume redirect',
      () async {
        final OnboardingTestStack stack = buildOnboardingStack();
        await stack.hydrate('u1');
        await stack.provider.advance();
  ...
  group('RouteGuard (server-authoritative completion)', () {
    ...
    test(
      'authoritative incomplete overrides a stale cached complete',
      () async {
        final OnboardingTestStack stack = buildOnboardingStack();
        await stack.hydrate('u1');
        await stack.provider.advance();
        ...
        expect(
          ...
          reason:
              'the authoritative incomplete wins over the stale cache, '
    ...
    test(
      'offline relaunch with cached completion degrades gracefully',
      () async {
        final OnboardingTestStack stack = buildOnboardingStack();
        await stack.hydrate('u1');
        for (int i = 0; i < 5; i++) {
