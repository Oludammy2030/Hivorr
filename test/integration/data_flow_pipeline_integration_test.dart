...
void main() {
  group('Data Flow Pipeline Integration (EP-01-20 §5.6)', () {
    test(
      '1. Cache-first read uses local datasource and avoids remote',
      () async {
    ...
    test(
      '2. Remote fetch writes through to local and loads into provider',
      () async {
        final FakeEntityRemoteDataSource remote = FakeEntityRemoteDataSource();
    ...
    test('3. Error propagation surfaces ApiException in provider', () async {
      ...
      expect(() => repository.getProfile('e1'), throwsA(isA<ApiException>()));
    ...
    test(
      '4. Provider emits idle -> loading -> loaded transitions and notifies',
      () async {
        final FakeEntityRemoteDataSource remote = FakeEntityRemoteDataSource();
        remote.profile = const EntityProfileDto(
          entityId: 'e1',
        ...
        expect(
          observed.indexOf(EntityProviderState.loading),
    ...
    test(
      '5. Update writes through to remote and local and updates provider',
      () async {
        final FakeEntityRemoteDataSource remote = FakeEntityRemoteDataSource();
        final FakeEntityLocalDataSource local = FakeEntityLocalDataSource();
        final EntityRepositoryImpl repository = EntityRepositoryImpl(
